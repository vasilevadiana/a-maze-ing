...
if __name__ == "__main__":
    ...
    while True:
        ...
        if user_input == 'q':
            ...
            maze = gen.generate(random.randint(0, 10000))  # generate a new maze with a random seed
            path = solve_maze(maze, start, exit_cell)
            print_maze(maze, path, config, show_path=False)
        elif user_input == 's':
            print("Showing path...")
            ...
            print_maze(maze, path, config, show_path=True, wall_color=input("Enter a new wall color (avalible colors: red, green, blue, orange): ").strip())
        else:
            print("Invalid input. Please try again.")

        #TODO change maze wall colors: line 52 (now it does nothing, but it should change the wall color of the maze based on user input)
